...
foo = htmlTable(outtable,
	...
	caption="Table xx.  Comparison of seasonal net radiation (Rn) from towers and GMAO during the validation period (Table 1) using nearest neighbor resampling.  Values extracted at the single MODIS pixel for each tower.",
	#rgroup = c("",""),
	#n.rgroup = c(7,2),
	#ctable=TRUE
)
...
setwd(outdir.tables)
sink("Table_2_Rn_tower_GMAO_compare_nobuffer_ngb.html")
