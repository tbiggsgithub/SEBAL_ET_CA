...
foo = htmlTable(outtable,
	...
	caption="Table xx.  Comparison of seasonal net radiation (Rn) from towers and GMAO during the validation period (Table 1) using bilinear interpolation of GMAO to the MODIS grid.  Values extracted for the MODIS cell at each tower.",
	#rgroup = c("",""),
	#n.rgroup = c(7,2),
	#ctable=TRUE
)
...
#	#header = c("Tower","GMAO","W/m2","%","","%")
#	header = c("Tower","GMAO","W m <sup>-2</sup>","%","W m <sup>-2</sup>","%"),
#	caption="Table xx.  Comparison of seasonal net radiation (Rn) from towers and GMAO during the validation period (Table 1).",
#)
#print(foo)
...
setwd(outdir.tables)
sink("Table_2_Rn_tower_GMAO_compare.html")
