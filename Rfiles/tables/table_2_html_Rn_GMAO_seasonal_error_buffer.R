...
fname.Rn = paste("Rn_GMAO_", as.character(year), "_UStowers_24hmean_buffer.txt",sep="")
Rn.GMAO = read.table(paste(indir.Rn,fname.Rn,sep=""))
Rn.GMAO.dates = strptime(Rn.GMAO$yyyyjjj,format="%Y%j")
tower = towers[i]
...
outtable = data.frame(stats[,1:2],Error=round(stats[,2]-stats[,1],1),Errorpct=errpct,RMSE=stats.df$RMSE.May.Oct,rRMSE=100*as.numeric(stats.df$rRMSE.May.Oct))
...
foo = htmlTable(outtable,
	...
	caption="Table xx.  Comparison of seasonal net radiation (Rn) from towers and GMAO during the validation period (Table 1) using bilinear interpolation of GMAO to the MODIS grid.  Means are over a 3x3 cell buffer around each tower.",
	#rgroup = c("",""),
	#n.rgroup = c(7,2),
	#ctable=TRUE
)
...
setwd(outdir.tables)
sink("Table_2_Rn_tower_GMAO_compare_buffer.html")
